...
#FDN 12.2018
#edited for South Pavilion BA 1.2019

setwd("P:/Sediment Analyses/Pollen/Data/SouthPavilion")
# setwd ("C:/Users/barendt/Documents")
...
pp1 <- read.csv('montSouthPavilion2018.csv', 
                stringsAsFactors = F)

# read the first three rows and transpose them into a data frame
labels <- read.csv('montSouthPavilion2018.csv', 
...
pdata<- read.table('montSouthPavilion2018.csv', 
                   header=F, skip=6, sep=",")
############
...
SPav <- filter(SPav, !is.na(StratOrder) )
# check sample sizes
...
SPav1Mat <- SPav1[,7:60] 
rownames(SPav1Mat) <- paste(SPav1[,4], SPav1[,2], sep = "-")
...
SPav2Mat <- dumpOutliers(SPav1Mat,c('2588F-S-27', '2588J-S-19', '2588F-S-21') )
...
Group <- c(rep('Herbs and Cultigens',29 ), rep('Arboreal', 55-30), 'Indeterminant')
